---
layout: default
title: About Me
permalink: /
description: Data engineer at the intersection of finance, analytics, and impact.
profile_image: /assets/images/IMG_9114.jpg
---

<div class="hero-profile reveal">
  <img 
    src="assets/images/IMG_9114.jpg" 
    alt="Vaishnavi Sreenivasa" 
  />
</div>

# Welcome 👋

<p class="typing-line reveal reveal-delay-1">
  I'm a <strong id="typing-role" data-phrases='["data engineer","analytics builder","pipeline architect","mountain trekker"]'></strong><span class="typing-cursor" aria-hidden="true"></span>
</p>

<div class="stats-grid reveal reveal-delay-2">
  <div class="stat-card" data-count="8" data-suffix="+">
    <span class="stat-value">0+</span>
    <span class="stat-label">Years Experience</span>
  </div>
  <div class="stat-card" data-count="20" data-suffix="M+">
    <span class="stat-value">0</span>
    <span class="stat-label">Users Impacted</span>
  </div>
  <div class="stat-card">
    <span class="stat-value">CMU</span>
    <span class="stat-label">Heinz · MISM</span>
  </div>
  <div class="stat-card">
    <span class="stat-value">SOX</span>
    <span class="stat-label">CCPA · Governance</span>
  </div>
</div>

Hi, I'm <strong>Vaishnavi</strong> — a data engineer and graduate student who enjoys building reliable data systems that turn complex information into clear, actionable insights.

I'm currently pursuing a <strong>Master of Information Systems Management at Carnegie Mellon University</strong>, and I bring <strong>8+ years of industry experience</strong> working across large-scale data platforms, analytics engineering, and financial systems. My work sits at the intersection of <strong>data engineering, product thinking, and decision-making</strong>, with a strong focus on trust, governance, and real-world impact.

<div class="cta-strip reveal">
  <a href="{{ '/projects' | relative_url }}" class="cta-btn cta-btn--primary">View Projects →</a>
  <a href="{{ '/contact' | relative_url }}" class="cta-btn cta-btn--secondary">Get in Touch</a>
</div>

---

## What I Work On

<div class="reveal" markdown="1">

Over the years, I've designed and built:

- **Scalable data platforms** supporting analytics, financial reporting, and compliance  
- **Distributed data pipelines** processing millions of records for product and customer insights  
- **SOX- and CCPA-compliant data systems** that balance speed with regulatory rigor  
- **Analytics-ready data models** that enable self-service reporting and experimentation  
- **AI-enabled internal tools**, including early prototypes using retrieval-augmented generation (RAG)

I've worked closely with engineers, analysts, finance teams, auditors, and product partners—often translating ambiguous business questions into robust technical solutions.

</div>

<div class="skill-section reveal">
  <h3>Tools & Technologies</h3>
  <div class="skill-tags">
    <span class="skill-pill">Python</span>
    <span class="skill-pill">SQL</span>
    <span class="skill-pill">Spark</span>
    <span class="skill-pill">Snowflake</span>
    <span class="skill-pill">AWS</span>
    <span class="skill-pill">Hive</span>
    <span class="skill-pill">Airflow</span>
    <span class="skill-pill">dbt</span>
    <span class="skill-pill">RAG / LLMs</span>
    <span class="skill-pill">Data Modeling</span>
    <span class="skill-pill">SOX Compliance</span>
    <span class="skill-pill">CCPA / Privacy</span>
  </div>
</div>

---

## How I Think About Data

<div class="reveal" markdown="1">

I care deeply about **data quality, usability, and accountability**. To me, good data engineering isn't just about moving data fast—it's about making sure people can *trust* it, *understand* it, and *use* it to make better decisions.

</div>

---

## Beyond Work

<div class="feature-banner reveal">
  <span class="feature-icon" aria-hidden="true">🏔️</span>
  <div>
    <h3>High-altitude adventures</h3>
    <p>I'm an avid trekker and have completed several high-altitude Himalayan treks in India and Nepal. I love adventures that challenge me and put me right in the middle of nature — the same discipline and patience I bring to the mountains shows up in how I build data systems.</p>
  </div>
</div>

<div class="reveal" markdown="1">

Outside of classes and projects, I enjoy mentoring, collaborating in team-based environments, and learning how technology intersects with organizational processes and public outcomes. I'm motivated by work that has **clarity of purpose**, whether that's improving financial transparency, enabling better products, or supporting more informed decision-making.

</div>

---

Feel free to explore my [projects]({{ '/projects' | relative_url }}) or [reach out]({{ '/contact' | relative_url }}) if you'd like to connect.
